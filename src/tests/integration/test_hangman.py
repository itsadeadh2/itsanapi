from src.infrastructure.exc import (
    GameOver,
)
...
class TestCreateGame(BaseHangmanTest):
    ...
    def test_return_new_game(
        ...
        return_data_valid = (
            True if not HangmanGameSchema().validate(response_data) else False
        )
        self.assertEqual(
            res.status_code,
            201,
    ...
    def test_require_auth(
        ...
        res = self.app.post("/api/games/hangman")
        self.assertEqual(
            res.status_code,
            401,
        )
    ...
    def test_return_500_on_error(
        self,
        hangman_handler_mock,
    ):
...
class TestGetAllGames(BaseHangmanTest):
    ...
    def test_return_all_games_from_user(
        self,
    ):
        ...
        user1_total_games = Faker().random_int(
            min=1,
            max=10,
        )
        ...
        user2_total_games = Faker().random_int(
            min=1,
            max=10,
        )
...
class TestTakeGuess(BaseHangmanTest):
    ...
    def test_return_422_if_guess_is_invalid(
        self,
    ):
        guess = {"guess": "morethanoneletter"}
        res = self.take_guess(
    ...
    def test_return_game_state_after_guess(
        self,
    ):
        guess = {"guess": Faker().random_letter()}
        res = self.take_guess(
            game_id=self.game_id,
            guess=guess,
        )
        ...
        is_game_data_valid = (
            True if not HangmanGameSchema().validate(res.get_json()) else False
        )
        self.assertEqual(
            True,
            is_game_data_valid,
    ...
    @patch("src.setup.injector.HangmanHandler")
    def test_return_400_if_game_is_over(
        self,
        hangman_handler_mock,
    ):
        self.setUp()
...
class TestGetLeaderboard(BaseHangmanTest):
    ...
    def test_return_entries_for_current_users(
        self,
    ):
        amount_of_users = Faker().random_int(
            min=10,
            max=20,
        )
